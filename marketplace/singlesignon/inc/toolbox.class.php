...
class PluginSinglesignonToolbox {
   ...
   public static function renderButton($url, $data, $class = 'oauth-login') {
      ...
      if (isset($data['picture']) && $data['picture']) {
         $btn .= Html::image(
            //Para insertar una imagen manualmente borrar la linea de abajo y insertar el enlace "url de imagen",
      ...
      //Aqui condicionamos para hacer que si lo que contiene data es GLPI el boton pase a decir Igresar con Usuario
      if (isset($data['name']) && $data['name'] === 'GLPI') {
         $btn .= sprintf(__sso('Ingresar con %s'), 'Usuario');
         //return '';
      } else {
         //Si no que el boton se llame como el servicio que se esta utilizando
         $btn .= sprintf(__sso('Ingresar con %s'), $data['name']);
      }
